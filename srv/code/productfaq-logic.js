...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "sameerahmed_1_a38Srv.ProductFAQ")
...
module.exports = async function(results, request) {
	...
	try {
		...
		let productFAQ = await SELECT.one('sameerahmed_1_a38.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		...
		await UPDATE("sameerahmed_1_a38.ProductFAQ", ID).with({ embedding: utils.array2VectorBuffer(embedding) });
